...
exports.update = async (req, res) => {
    ...
    PrivacyPolicy.update(privacy_policy, {
      where: { id: id }
    })
    .then((num) => {
      console.log("num", num);
      if (num == 1) {
        RESPONSE.Success.Message = "Privacy policy was updated successfully.";
        RESPONSE.Success.data = { id: id };
        res.status(StatusCode.CREATED.code).send(RESPONSE.Success);
      } else {
        RESPONSE.Failure.Message = `Cannot update Privacy policy with id=${id}. Maybe Privacy policy was not found or req.body is empty!`;
